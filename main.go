...
var (
	...
	// deviceModel = prometheus.NewGaugeVec(
...
func recordMetrics(upscBinary string, upsArg string) {
	...
	// prometheus.MustRegister(deviceModel)
	...
	go func() {
		for {
			...
			// if deviceModelRegex.FindAllStringSubmatch(string(upsOutput), -1) == nil {
			// 	prometheus.Unregister(deviceModel)
			// } else {
			// 	deviceModelValue, _ := strconv.ParseFloat(deviceModelRegex.FindAllStringSubmatch(string(upsOutput), -1)[0][1], 64)
			// 	deviceModel.WithLabelValues(deviceModelValue).Set(1)
			// }
